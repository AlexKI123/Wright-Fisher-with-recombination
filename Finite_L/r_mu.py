...
@nb.njit
def pfunc_onlyviables(r, mu, N, l, viables, two_point_distances, Min_mutations, Visited):
    gen_counter_avg = 0
    mut_counter_avg = 0
    m_value_avg = 0.0
    avg_hd_avg = 0.0
    nogenotypes_avg = 0.0
    nogenotypes_viable_avg = 0.0
    nosegmut_avg = 0.0
    meanfitness_avg = 0.0
    ...
    while missing_genotypes > 0 or Min_mutations > mut_counter:
        gen_counter += 1
        ...
        nogenotypes_viable += 1.0 * temp_values[5]
    ...
    nogenotypes_viable_avg += 1.0 * (nogenotypes_viable / gen_counter)


    returnarray = numpy.zeros(9)
    ...
    returnarray[8] = nogenotypes_viable_avg
...
@nb.njit
def pfunc(r, mu, N, l, viables, lethals, two_point_distances, Min_mutations, Visited):
    gen_counter_avg = 0
    mut_counter_avg = 0
    m_value_avg = 0.0
    avg_hd_avg = 0.0
...
@nb.njit
def pfunc_sel_rec(r, mu, N, l, viables, two_point_distances, Min_mutations):
    gen_counter_avg = 0
    mut_counter_avg = 0
    m_value_avg = 0.0
    avg_hd_avg = 0.0
...
def selrec_r_mu_cluster(r, mu, N, l, p, k, avg, landscape, two_point_distances, Min_mutations, Visited):
    ...
    for _ in range(avg):
        ...
        if p<1.0:
            returnarray = pfunc(r, mu, N, l, viables, lethals, two_point_distances, Min_mutations, Visited)
            #returnarray = pfunc_sel_rec(r, mu, N, l, viables, two_point_distances, Min_mutations)
        else:
            returnarray = pfunc_onlyviables(r, mu, N, l, viables, two_point_distances, Min_mutations, Visited)
        ...
        nogenotypes_viable_avg +=returnarray[8]/avg

    return gen_counter_avg, mut_counter_avg, m_value_avg, avg_hd_avg, nogenotypes_avg, nosegmut_avg, rr_value_avg, meanFitness_value, nogenotypes_viable_avg
...
tm_array = numpy.zeros((1, 11))
tm_array[0, 0] = r
tm_array[0, 1] = mu

start = timeit.default_timer()
temp_values = selrec_r_mu_cluster(r, mu, N, l, p, k, avg, landscape, two_point_distances, Min_mutations, Visited) #attention can also be sel_rec
...
numpy.save('L{}_N{}_p{}_avg{}_dp{}_vi{}_mM{}_i{}_{}.npy'.format(l, N, p, avg, datapoints, Visited, Min_mutations, int(x1), landscape), tm_array)
