...
#################################################################################
#################################################################################
# Code to plot Fig. S10
#################################################################################
#################################################################################

rmin = 0.001
rmax = 1.0
mumin= 0.0001  #0.0001 1e-06
mumax= 0.5
datapoints = 30 #30
avg = 10000 #1000 #10000
Min_mutations = 200000 #20000
Visited = 1
log = True
l = 10  #6  10
N = 100
...
tm_array=numpy.load('saved_data/RMU_viabledistinct_L{}_N{}_p{}_avg{}_dp{}_rmin{}_rmax{}_mumin{}_mumax{}_vi{}_mM{}_{}.npy'.format(l, N, p, avg, datapoints, rmin, rmax, mumin, mumax, Visited, Min_mutations, landscape))
...
if plotno == 4 or plotno == 5 or plotno == 6 or plotno ==7:
    ax.set_xlim(ax.get_xlim()[0], np.log10(rmax))
    ax.set_ylim(ax.get_ylim()[0], np.log10(mumax)+0.01)
...
if plotno == 7:
    if logz == False:
        ...
        ax.contour(x, y, z, 5, cmap="plasma", linestyles="solid", offset=ax.get_zlim()[0]-0.05)
if plotno == 8:
    ax.contour(x, y, z, 5, cmap="plasma", linestyles="solid", offset=ax.get_zlim()[0]-0.01)
if plotno == 9:
    ax.contour(x, y, z, 5, cmap="plasma", linestyles="solid", offset=ax.get_zlim()[0] - 0.02)
...
if plotno == 5:
    ...
    #else:
    #    ax.plot(np.log10(rrange), np.log10(murange), np.log10(2*(murange-1)*murange*l*N/(4*(murange-1)*murange*(N-1)-1)), linewidth=3.0, color="C0", zorder=10)
...
if logz is False:
    ...
    if p==1:
        ax.plot(np.log10(np.array(UlineList_r)), np.log10(np.array([Uline]*len(UlineList_r))), np.array(UlineList_Z), color="C9", linewidth=3.0, zorder=100)
else:
    if p==0.5:
...
if plotno == 4:
    '''
    ax.view_init(15.52937, -137.8234)
    fig.subplots_adjust(left=-0.01)
...
if plotno == 8:
    ax.view_init(15.52937, 123.7060)
...
if plotno == 4:
    plt.savefig('FSM_p{}_m_{}.pdf'.format(p, landscape))
if plotno == 5:
