...
from modules import clean_fixed_mutations_m7
...
if load is False:
    ...
    if save is True:
        if model == 1:
            np.save('saved_data/Cloud_CrossSection_N{}_U{}_r{}_gen{}.npy'.format(N, U, r, generations), d1_distribution_list)
        if model == 2:
    ...
    if both is False:
        if model ==1:
            d1_distribution_list = np.load('saved_data/Cloud_1dshape_N{}_U{}_r{}_gen{}.npy'.format(N, U, r, generations))
        if model == 2:
        ...
        if model == 1:
            d1_distribution_list_r0 = np.load('saved_data/Cloud_1dshape_N{}_U{}_r{}_gen{}.npy'.format(N, U, 0.0, generations))
            d1_distribution_list_r1 = np.load('saved_data/Cloud_1dshape_N{}_U{}_r{}_gen{}.npy'.format(N, U, 1.0, generations))
        if model == 2:
...
if both is False:
    ...
    plt.title("$N={}$".format(N) + ", $U={}$".format(U) + ", $N U={}$".format(N*U) + ", model=${}$".format(model))
...
if model == 1:
    plt.plot(x_r1, poisson_y, label="$h(d,r=0)$", color="C1")
if model == 2:
...
if model == 1:
    plt.semilogy(x_r1, poisson_y, label="$h(d,r=1)$", color="C1")
if model == 2:
