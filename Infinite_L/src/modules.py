...
fitness_lethal = 0.8
def selrec_vl_v2_select_lethal(wp, N, r, p, viables, lethals):
    viable_counter = 0
    lethal_counter = 0
    novel_counter = 0
    recomb_counter = 0
    ...
    ###mean_Fitness = (1.0 * sum([uniques[x] for x in distinct_genotypes if x in viables]) + fitness_lethal * sum([uniques[x] for x in distinct_genotypes if x in lethals])) / N
    ...
    for x1 in range(N):
        if random.random() <= r:
            ...
            if tuple(sorted(wp[randint1])) in lethals:
                if random.random() > fitness_lethal:
                    select_lethal1 = 0
            if tuple(sorted(wp[randint2])) in lethals:
                if random.random() > fitness_lethal:
                    select_lethal2 = 0

            while randint1 == randint2 or select_lethal1 == 0 or select_lethal2 == 0:
                randint1 = random.randint(0, N - 1)
                randint2 = random.randint(0, N - 1)
                select_lethal1 = 1
                select_lethal2 = 1
...
# one generation with concurrent recombination and lethal genotypes. Counting viable recombination fraction
...
# one generation with concurrent recombination (single-crossover) and lethal genotypes
# S5 Fig left -- each novel mutation is assigned a genomic position as a uniform random variable within the range [0,1), and the position of the crossover is determined by another random number within this range.
def selrec_vl_v2_SC(wp, N, r, p, viables, lethals):
    viable_counter = 0
    lethal_counter = 0
    novel_counter = 0
    recomb_counter = 0
    ...
    for x1 in range(N):
        if random.random() <= r:
            ...
            a = set([i for i in a if i <= crossover_point])
            b = set([i for i in b if i > crossover_point])
            c = a ^ b
            wp_temp[x1] = c.copy()
            if tuple(sorted(wp_temp[x1])) in viables:
                viable_counter += 1
...
# one generation with concurrent recombination and lethal genotypes
# S6 Fig left -- all individuals recombine and r describes instead the average number of crossover
def selrec_vl_v2_OBLIGATORY_Crossovers(wp, N, r, p, viables, lethals):
    viable_counter = 0
    lethal_counter = 0
    novel_counter = 0
    recomb_counter = 0
    ...
    for x1 in range(N):
        ...
        if random.random() > 0.5: ##unnescessary
            a = wp[randint1]
            b = wp[randint2]
        else:
            b = wp[randint1]
        ...
        c = set()
        crossovers = np.random.poisson(r)
        if crossovers > 0:
            crossover_points = sorted([random.random() for i in range(crossovers)])
            c.update([i for i in a if i <= crossover_points[0]])
            if crossovers > 1:
                for crossing in range(1, crossovers):
                    if crossing & 1:
                        c.update([i for i in b if
                                  i > crossover_points[crossing - 1] and i <= crossover_points[crossing]])
                    else:
                        c.update([i for i in a if
                                  i > crossover_points[crossing - 1] and i <= crossover_points[crossing]])
            if crossovers & 1:
                c.update([i for i in b if i > crossover_points[crossovers - 1]])
            else:
                c.update([i for i in a if i > crossover_points[crossovers - 1]])
        else:
            c.update(a)
...
# removing fixed mutations to free memory
...
# concurrent recombination for #generations (uniform crossover || single-point crossover || obligatory recombination)
def selrec_r_U_cluster(r, U, N, generations, discarded_elements, p):
    ...
    for gen in range(generations):
        ...
        #wp, viables, lethals, viable_counter_temp, lethal_counter_temp, novel_counter_temp, recomb_counter_temp = selrec_vl_v2_SC(wp, N, r, p, viables, lethals)
        #wp, viables, lethals, viable_counter_temp, lethal_counter_temp, novel_counter_temp, recomb_counter_temp = selrec_vl_v2_OBLIGATORY_Crossovers(wp, N, r, p, viables, lethals)
...
def selrec_r_U_cluster_select_lethals(r, U, N, generations, discarded_elements, p):
    ...
    for gen in range(generations):
        #print("gen", gen)
        # Evolution
        wp, viables, lethals, viable_counter_temp, lethal_counter_temp, novel_counter_temp, recomb_counter_temp = selrec_vl_v2_select_lethal(wp, N, r, p, viables, lethals)
        wp, mutation_counter, viables, lethals = mutation_vl_poisson(wp, N, U, p, mutation_counter, viables, lethals)
        ...
        ###mean_Fitness[gen] = 1.0*sum([True for x in wp if tuple(sorted(x)) in viables])/N
        #mean_Fitness[gen] = 1.0*sum([uniques[x] for x in distinct_genotypes if x in viables]) / N
        mean_Fitness[gen] = (1.0*sum([uniques[x] for x in distinct_genotypes if x in viables]) + fitness_lethal*sum([uniques[x] for x in distinct_genotypes if x in lethals])) / N
